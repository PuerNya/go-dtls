...
| `EnableGREASE` | `false` by default; sends one randomized empty RFC 8701 extension in client CH and server CR/NST; it is not negotiated and remains unchanged across HRR and ECH Inner/Outer cloning |
...
| [RFC 8701](https://www.rfc-editor.org/rfc/rfc8701) | GREASE anti-ossification | The extension-value strategy is complete: `EnableGREASE` sends a randomized empty extension in CH/CR/NST, receivers ignore it without negotiation state, and HRR preserves the value; other MAY injection points are not actively sent |
...
- RFC 8701 tests cover fixed-random mapping, CH/CR/NST wire encoding, the final-PSK constraint, HRR, mTLS/PHA, session tickets, exclusion from negotiation state, and real-UDP wolfSSL tolerance of all three extension locations.
